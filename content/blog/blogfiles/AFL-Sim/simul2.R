...
Finalsimtable <- cbind.data.frame(MeanPos = colMeans(SimResultspois),
                                  MedianPos = apply(SimResultspois, 2, median),
                                  BestPos = apply(SimResultspois, 2, min),
                                  WorstPos = apply(SimResultspois, 2, max),
                                  percentWon =  apply(SimResultspois == 1, 2, sum)/nrow(SimResultspois)*100,
                                  percentTop4 = apply(SimResultspois <= 4, 2, sum)/nrow(SimResultspois)*100,
                                  percentTop8 = apply(SimResultspois <= 8, 2, sum)/nrow(SimResultspois)*100,
                                  percentBottom4 = apply(SimResultspois > 14, 2, sum)/nrow(SimResultspois)*100,
                                  percentLast = apply(SimResultspois == 18, 2, sum)/nrow(SimResultspois)*100) %>%
                 cbind.data.frame(Team = strengths$Team) 

Finalsimtablepois <- Finalsimtable[,c(10, 2, 1, 3:9)] %>% arrange(MedianPos, MeanPos) %>% tibble() 
Finalsimtablepois
